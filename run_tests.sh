...
# and per-app lint/typecheck/build) in sequence, prints a pass/fail summary, and
# exits non-zero on any failure. There is no CI pipeline (see CLAUDE.md), so
...
# tests/frontend/demo.test.ts — both are picked up automatically, no edit to
...
run_frontend_unit() {
  ...
  run_step "tests/frontend/demo.test.ts" node demo.test.ts
  popd > /dev/null
}
...
run_frontend_app_checks() {
  ...
  echo ""
  echo "################################"
  echo "# Frontend app: $app"
  echo "################################"
  pushd "frontend/apps/$app" > /dev/null || return 1
  run_step "$app: npm run lint" npm run lint
  run_step "$app: tsc --noEmit" npx tsc --noEmit
  run_step "$app: npm run build" npm run build
  popd > /dev/null
}
...
case "$MODE" in
  ...
  --frontend)
    run_frontend_unit
    run_frontend_app_checks client
    run_frontend_app_checks admin
    ;;
  --unit)
    run_backend
    run_frontend_unit
