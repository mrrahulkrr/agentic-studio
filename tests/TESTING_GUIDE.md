...
[`TEST_PLAN.md`](TEST_PLAN.md) in this same directory — this guide is the
*how*.

There is no CI (see `CLAUDE.md`) and no test framework — every check is a
...
- **Python**, with the backend's dependencies already installed (`pip
  ...
  (`app.core.auth`, `app.data.database`, …), just with network/DB calls
  monkeypatched out — they still need the packages those modules import at
  module load time (FastAPI, psycopg2, etc.).
- **Node.js ≥ 22.6** for the one frontend test file — that version is the
...
that would call out — see the "Scope" note in each file's docstring for
...
the end, and **exits non-zero if anything failed** — safe to use as a gate
before committing.
...
frontend logic — it's the whole plain-assert suite with none of the slower
Next.js tooling.
...
itself (see §5), so you can run them from anywhere by path, or from inside
`tests/backend/`:
...
the frontend file) and exits non-zero — `run_tests.sh` treats any non-zero
...
- **An `ImportError` / `ModuleNotFoundError` for `app.*`** almost always
  ...
  copied a test file out of `tests/backend/` without keeping that fixup —
  see §5.
- **A monkeypatch that silently "worked" but nothing changed** — check that
...
- **`demo.test.ts` throwing on import** — check the three relative imports
...
(`tests/backend/test_x.py` → `parents[2]` is the repo root → `/ "backend"`),
...
`tests/backend/` on `sys.path`, not `backend/` — the `cd` wouldn't fix
...
## 6. Writing a new test — the house style
...
3. **Small, explicit fixtures** built right next to the test that uses them
   — hand-built dicts/objects, not shared setup/teardown machinery. There's
...
7. **Monkeypatch by name on the module under test**, not on the module that
   defines the function — see the note in §4. Restore the original in a
...
**Before adding a check, search for it first** — don't duplicate a case
...
for the test suite itself, ever, without asking first — the no-framework
...
app's `npm run lint` / `npx tsc --noEmit` / `npm run build` — `./run_tests.sh
...
cd tests/frontend && node demo.test.ts             # the frontend file
