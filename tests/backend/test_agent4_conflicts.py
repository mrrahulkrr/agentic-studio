...
rot silently" — that gap is characterized directly below rather than only
...
def test_a_failed_fetch_for_the_primary_year_is_reported_as_unknown_not_ok():
    ...
    'no conflict' — 'unknown' has to reach the caller distinguishably."""
    patch = _Patcher()
    try:
        patch.setattr(agent4, "SUPPORTED_COUNTRIES", ["US"])
...
def test_known_gap_a_year_with_no_hardcoded_entry_silently_uses_the_nearest_other_year():
    ...
    There is no explicit 'unknown year' case — a date far outside the
    ...
    # Nearest hardcoded date is 2030-07-21 (there is no 2031 entry at all) —
