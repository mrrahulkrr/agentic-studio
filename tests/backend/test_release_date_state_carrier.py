...
uses `results.script_text` as a state carrier ΓÇö `release_check` results store
...
def test_known_gap_shifted_date_can_land_on_a_second_unrelated_conflict():
    ...
    them. This is not a desired behavior ΓÇö it's here so a future fix to
    ...
    # conflicting date, this assertion should start failing ΓÇö replace it with
