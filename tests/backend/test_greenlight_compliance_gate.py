"""Checks for app/ai/agents.py::check_compliance_structured — the Greenlight
...
Postgres. guardrails.retrieval_status() itself is NOT patched — it runs for
...
indistinguishable — both produced "message": "No guidelines found." with no
...
def _fake_generate_text(*responses):
    ...
    asked something — specifically, that the violations prompt is skipped
    entirely for "empty"/"low_relevance" but not for "unscored".

    Stands in for generate_for_tier, not generate_text directly — its first
...
def test_genuinely_empty_collection_returns_the_empty_message_without_checking_violations():
    ...
    try:
        ...
        # Only the flagged-topics call happened — never asked to find violations
...
def test_reranker_failure_produces_unscored_not_empty_and_still_checks_the_retrieved_guidelines():
    ...
    knowledge base) but the reranker failed, so every rerank_score is None —
    ...
    try:
        ...
        # violation surfaced — the old gate would have returned [] here.
        ...
        # And the caveat says *unscored*, not *empty* — the two must not read
