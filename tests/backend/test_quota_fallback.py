...
agents.py/ingest.py/retrieval.py/evaluator.py all run unmodified ΓÇö this is
...
class _FakeModels:
    ...
    RESOURCE_EXHAUSTED/UNAVAILABLE errors do ΓÇö generate_text only inspects
...
def test_a_503_response_still_retries_and_still_falls_back_to_the_generic_string():
    """This task changed handling for 429/RESOURCE_EXHAUSTED only ΓÇö 503's
...
# ---- isolation ΓÇö at least two different tiers, per two different files ----
...
def _fake_quota_generate_text(system_prompt, user_prompt, model=None, **kwargs):
    ...
    what attaches `tier` ΓÇö this is what actually proves the wrapper is wired
...
def test_gemini_rerank_degrades_to_unscored_on_quota_exhaustion_rather_than_propagating():
    ...
    fail the whole retrieval. Quota exhaustion ΓÇö the highest-volume tier's
    most likely failure ΓÇö is just one more reason that contract already
...
def test_score_faithfulness_degrades_to_a_none_scored_result_on_quota_exhaustion():
    ...
    quota included, returns EvalResult(score=None, ...) instead of raising ΓÇö
...
def test_check_compliance_flag_topics_call_degrades_via_safe_generate():
    ...
    fallback string ΓÇö quota exhaustion included."""
    patch = _Patcher()
    try:
        patch.setattr(llm, "generate_text", _fake_quota_generate_text)
        patch.setattr(agents, "hybrid_search", lambda *a, **k: [])
        result = agents.check_compliance("INT. WAREHOUSE - NIGHT. A scene.")
        # Falls through to the "empty" retrieval_status branch ΓÇö the point
...
def test_a_model_not_on_that_tiers_candidate_list_is_rejected():
    try:
        # gemini-2.5-pro is a real, free model ΓÇö just not an approved FAST
