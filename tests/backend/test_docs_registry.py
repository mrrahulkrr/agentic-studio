...
`key` and a filesystem read — see docs_registry.py's own module docstring.
...
hardcoded expected string could), (3) an unknown key — including a path-
traversal attempt — is refused the same way any other unknown key is,
...
— require_api_key and require_role are called directly as the dependency
...
def test_valid_key_returns_the_actual_file_content():
    ...
    # pass by drifting alongside a hardcoded expected string — only by the
...
def test_admin_doc_endpoint_404s_on_traversal_attempt_not_500_or_empty_200():
    ...
    # HTTPException(404), the same path an ordinary unknown key takes —
...
def test_rejected_for_a_non_developer_role():
    ...
    # This just confirms a client-role user does not pass it here either —
