...
// into frontend/src/lib/ since the source it tests still lives there.
...
import {
  ...
  subscribeDemo,
  type WalkthroughId,
} from "../../frontend/src/lib/demo.ts";
import {
  TIER_CANDIDATES,
  TIER_DEFAULT_MODELS,
  TIER_LABELS,
  WALKTHROUGHS,
} from "../../frontend/src/lib/content.ts";
import { describeRequest } from "../../frontend/src/lib/activity.ts";
import {
  MASK,
  clearApiLog,
  formatResponse,
  ...
  maskSecrets,
  setApiLogVisible,
} from "../../frontend/src/lib/apilog.ts";
...
assert.strictEqual(
  ((await demoRequest("/admin/tables/results/4101", {})) as { row: { script_text: string } }).row.script_text,
