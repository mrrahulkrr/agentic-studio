...
framework (no pytest, no vitest) — every file runs standalone with a bare
...
|---|---|---|
| Release-date conflict logic + retrieval confidence gate | `test_release_conflicts.py` | Pure date arithmetic that replaced an LLM call specifically because the LLM could hallucinate a film — a regression here reintroduces silent wrong answers. `retrieval_status()`'s four states are easy to collapse back into a boolean by accident. |
...
| Login sessions / role gate | `test_auth.py` | Password hashing, JWT roundtrip/tamper, and `require_role`'s re-check-on-every-call (not just the JWT claim) — the mechanism that stops a demoted or deleted account from keeping developer access for up to 12h. |
...
| Greenlight Committee debate graph | `test_greenlight_committee.py` | The one task (`greenlight`) that isn't a single routed node — a whole separate multi-node LangGraph with its own stalemate/iteration-cap logic and RED/YELLOW/GREEN verdict rules. |
...
| Calendar MCP → service-account silent fallback | `test_calendar_fallback.py` | `CALENDAR_MODE=mcp` failures are swallowed and silently retried on the service-account path — worth pinning down that failures of the *fallback itself* are not also swallowed. |
...
| Rate limiter / retry / safe-generate fallback | `test_resilience.py` | Shared in-process state used by `/run-agent`, `/check-conflicts`, `/finalize-calendar`, and `/auth/login` — a bug here is a bug on all of them simultaneously. |
...
| Demo Mode, walkthroughs, activity narration, API-key masking | `tests/frontend/demo.test.ts` | Demo Mode's contract is "every endpoint has a fixture, or it throws" — an easy thing to silently violate when adding an endpoint. |
...
checks. All six are covered by the table above — no gap was found when this
...
directory, not the process's working directory — a `cd backend && python
...
- Backend: drop a `tests/backend/test_*.py`, plain asserts, no framework,
  ...
  `tests/backend/test_*.py` automatically — no edit to the script needed.
...
in this task's stated scope) — see the migration notes for this move.
