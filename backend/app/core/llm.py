...
def generate_for_tier(
    ...
    *,
    ...
    Two things live here so none of the 12 call sites repeat them:
    ...
    Whether the exception then reaches the caller's caller (and eventually
    a user-facing rate-limit dialog) or gets swallowed into an existing
    graceful-degradation path (safe_generate, gemini_rerank's fallback to
    unscored, score_faithfulness's fallback EvalResult) is entirely up to
    each call site's own, unchanged exception handling — this function
    doesn't decide that, it only makes sure `tier` is on the exception by
    the time any of them see it.
    """
    model = model_override or TIER_MODELS[tier]
    try:
